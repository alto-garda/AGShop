...
import { CambiaTaglia } from "@/components/ordini/CambiaTaglia";
...
export default async function OrdinePage({ params }: Props) {
  ...
  const tagliePerArticolo = new Map<
    string,
    { taglia: string; giacenza: number }[]
  >();

  for (const item of disponibilita ?? []) {
    const lista =
      tagliePerArticolo.get(item.articolo_id) ?? [];
    ...
    tagliePerArticolo.set(item.articolo_id, lista);
  }
